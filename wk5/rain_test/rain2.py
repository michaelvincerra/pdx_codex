...
def processor():
    ...
    # Create separate FUNCTION BELOW -- TEST IT
    ...
    data_dict = {rain_day[0]: (int(rain_day[1]), rain_day[2:]) for rain_day in lines if
                 '-' not in rain_day[1:]}  # set rain_day[0] as key, which is ('DATE')

    max_rain = max(data_dict.items(),
                   key=lambda t: t[1][0])  # data_dict.items() creates a list of tuples as a generator
